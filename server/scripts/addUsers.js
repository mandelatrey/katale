...
// existing data (transactions, payments, carriers keep their string
// buyer/seller/paidBy/paidTo fields intact). It only:
//
//   1. Ensures the `users` collection exists with the right indexes.
//   2. Creates the new indexes on Transaction / Payment / Carrier that
//      the updated schemas declare.
...
//           RUN_DEV_SEED=1 npm run migrate:users    (adds 3 demo users)
...
async function main() {
  ...
  console.log("[migrate:users] synced indexes on User / Transaction / Payment / Carrier");
  ...
  const counts = {
    ...
    carriersWithOwner: await Carrier.countDocuments({
      owner: { $exists: true, $ne: null },
    }),
  };
  console.log("[migrate:users] counts:", counts);

  // 3. Optional dev seed — three archetype users with fake E.164 numbers.
  if (process.env.RUN_DEV_SEED === "1") {
    const seed = [
      ...
      { name: "Okello P. (Carrier)", phoneE164: "+256700000003", role: "carrier" },
    ];
    for (const u of seed) {
